...
class MultiStepWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        ############
        # NOTE : AsnycVectorEnv raise error when episode is truncated or terminated.
        #        So we need to fill the remaining steps with 0.
        remaining_steps = self.n_action_steps - len(states)
        if remaining_steps > 0:
            # states가 비어있지 않으면 마지막 state의 shape을 참조, 아니면 빈 배열
            if states and len(states) > 0:
                zero_state = np.zeros_like(states[0]) if hasattr(states[0], 'shape') else []
            else:
                zero_state = []
    ...
    def set_episode_number(self, episode_number: int):
        """Set the current episode number for video naming."""
        # 하위 wrapper로 전달
        if hasattr(self.env, 'set_episode_number'):
            self.env.set_episode_number(episode_number)
