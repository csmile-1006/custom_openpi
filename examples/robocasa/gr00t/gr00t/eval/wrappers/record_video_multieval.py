...
class RecordVideo(gym.Wrapper[ObsType, ActType, ObsType, ActType], gym.utils.RecordConstructorArgs):
    ...
    def stop_recording(self):
        ...
        # 너무 적은 프레임은 저장하지 않음 (1프레임 이하)
        if len(self.recorded_frames) <= 1:
            logger.warn(f"Ignored saving a video as there were only {len(self.recorded_frames)} frames to save.")
        else:
            try:
    ...
    def set_episode_number(self, episode_number: int):
        ...
        self.episode_id = episode_number - 1  # reset에서 +1 되므로 미리 -1
