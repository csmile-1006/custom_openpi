...
def best_of_n_action_selection_mix(
    action_tokens: np.ndarray, actions: np.ndarray, log_probs: np.ndarray, kl_divs: np.ndarray, strategy: str
):
    # action tokens : (N, B, 256) or (B, 256)
    while action_tokens.ndim < 3:
        action_tokens = action_tokens[None, ...]
    while actions.ndim < 3:
        actions = actions[None, ...]
    while log_probs.ndim < 3:
        log_probs = log_probs[None, ...]
    while kl_divs.ndim < 3:
        kl_divs = kl_divs[None, ...]
    ...
    # mask: (N, B, 256)
    mask = (indices[None, None, :] >= 8) & (indices[None, None, :] < action_eos[..., None])
    masked_log_probs = np.where(mask, log_probs, 0)  # (N, B, 256)
    masked_kl_divs = np.where(mask, kl_divs, 0)  # (N, B, 256)

    # NOTE: robocasa's first 8 outputs is dummy (3 : "Action :", 5 : base_motion + control_mode)
    if strategy == "sum":
        log_prob_scores = masked_log_probs.sum(axis=-1)  # (N, B)
        kl_scores = masked_kl_divs.sum(axis=-1)  # (N, B)
    elif strategy == "avg":
        num_tokens = np.clip(action_eos - 8, 1, T)  # (N, B)
        log_prob_scores = masked_log_probs.sum(axis=-1) / num_tokens  # (N, B)
        kl_scores = masked_kl_divs.sum(axis=-1) / num_tokens  # (N, B)
    elif strategy == "first10":
        log_prob_scores = masked_log_probs[..., 8:18].sum(-1)  # (N, B)
        kl_scores = masked_kl_divs[..., 8:18].sum(-1)  # (N, B)
    elif strategy == "first5":
        log_prob_scores = masked_log_probs[..., 8:13].sum(-1)  # (N, B)
        kl_scores = masked_kl_divs[..., 8:13].sum(-1)  # (N, B)
    else:
        raise ValueError(f"Unknown strategy: {strategy}")

    # 각 스코어를 정규화 (0~1 범위로)
    def min_max_norm(scores):
        min_vals = np.min(scores, axis=0, keepdims=True)  # (1, B)
        max_vals = np.max(scores, axis=0, keepdims=True)  # (1, B)
    ...
    # log_prob_scores와 kl_scores를 각각 정규화
    norm_log_prob_scores = min_max_norm(log_prob_scores)  # (N, B)
    norm_kl_scores = min_max_norm(kl_scores)  # (N, B)

    # 1:1로 더해서 최종 스코어 계산
    final_scores = norm_log_prob_scores + norm_kl_scores  # (N, B)

    # Select best actions for each batch
    best_actions = []
    best_indices = []

    for b in range(final_scores.shape[1]):  # 각 배치에 대해
        batch_scores = final_scores[:, b]  # (N,)
        batch_log_probs = log_prob_scores[:, b]  # (N,) - 동률 처리용

        # 동률 처리: final_scores가 동률일 때 log_prob이 높은 것을 선택
        max_score = np.max(batch_scores)
        max_indices = np.where(batch_scores == max_score)[0]

        if len(max_indices) == 1:
            best_index = int(max_indices[0])
        else:
            # 동률인 경우 log_prob이 높은 것을 선택
            tie_log_probs = batch_log_probs[max_indices]
            best_tie_idx = max_indices[np.argmax(tie_log_probs)]
    ...
    return best_actions, best_indices, final_scores


def analyze_existing_videos(
    video_path: str, env_name: str, n_envs: int, n_episodes: int
    ...
    episodes_to_remove = [ep for ep in existing_episode_results if ep not in video_episodes]
    ...
    if existing_episode_results:
        ...
        for env_idx in range(n_envs):
            ...
            logging.info(
                f"Environment {env_idx}: {skip_episodes_per_env[env_idx]} valid episodes (range: {env_start_episode}-{env_end_episode - 1})"
            )

        # existing_episode_results도 실제 비디오가 있는 것만으로 업데이트
        existing_episode_results = {
            ep: existing_episode_results[ep] for ep in existing_episode_results if ep in actual_video_episodes
        }
    else:
...
def generate_episode_seed(base_seed: int, episode_number: int) -> int:
    """각 episode별로 고유한 seed를 생성"""
    return base_seed + episode_number
...
class Args:
    ...
    # BoN
    reward_strategy: str | None = None  # "log_likelihood", "negative_entropy"
...
def eval_robocasa(args: Args) -> None:
    ...
    skip_episodes_per_env, existing_episode_results = analyze_existing_videos(
        args.video_path, args.env_name, args.n_envs, args.n_episodes
    )
    ...
    while completed_episodes < args.n_episodes:
        ...
        # 현재 episode별 seed 계산
        current_episode_seeds = []
        for env_idx in range(args.n_envs):
            episode_seed = generate_episode_seed(args.seed, episode_numbers[env_idx])
            current_episode_seeds.append(episode_seed)
        ...
        element = {
            ...
            "annotation.human.action.task_description": [
                obs["annotation.human.action.task_description"][i][0] for i in range(args.n_envs)
            ],
            "episode_seeds": np.array(current_episode_seeds, dtype=np.int32),  # Episode별 seed 추가
        }
        ...
        if args.reward_strategy is not None:
            if args.reward_strategy == "negative_entropy":
                if "entropies" not in outputs:
                    raise ValueError(
                        "To use 'negative_entropy' with the default policy, server must be run with --policy.reward_strategy=\"negative_entropy\""
                    )
            ...
            elif args.reward_strategy == "likelihood+kl_div":
                action_tokens = outputs["action_tokens"]
                actions = outputs["actions"]
                log_probs = outputs["log_probs"]
                kl_divs = outputs["kl_divs"]
            ...
            if args.reward_strategy == "likelihood+kl_div":
                action_chunk, best_index, scores = best_of_n_action_selection_mix(
                    action_tokens, actions, log_probs, kl_divs, strategy=args.best_of_n_strategy
                )
            else:
                action_chunk, best_index, scores = best_of_n_action_selection(
                    action_tokens, actions, rewards, strategy=args.best_of_n_strategy
                )
        ...
        action_dict = {
            "action.base_motion": action_chunk[..., :4],  # (B, 16, 4)
        ...
        for env_idx in range(args.n_envs):
            ...
            if terminations[env_idx] or truncations[env_idx]:
                ...
                # 즉시 파일에 저장 (재시작 대비)
                save_episode_result(
                    f"{args.video_path}/prediction.txt", episode_numbers[env_idx], current_successes[env_idx]
                )
    ...
    assert completed_episodes >= args.n_episodes, f"Expected {args.n_episodes} episodes, got {completed_episodes}"
